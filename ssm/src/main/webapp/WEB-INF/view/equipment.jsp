...
<body class="gray-bg">
        ...
        <div class="row">
			  ...
			  <div class="col-lg-2">
				<div class="input-group">
					...
					<select class="form-control" name="status" id = "a">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveEquipment.htm">
						<input type="hidden" name="eid" id="hidden_txt_userid" value=""/>
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>设备名称：</td>
								<td><input type="text" id="username" name="ename"
							...
							<tr>
								<td>内存：</td>
								<td><input type="text" id="memory" name="memory"
									class="form-control" aria-required="true" required/></td>
								<td>&nbsp;&nbsp;</td>
								<td>机身颜色：</td>
								...
								<td colspan="2">
									<select class="form-control" name="mid" id = "mid" aria-required="true" required>
										...
										<c:forEach items="${mList }" var="m">
										 	<option value="${m.mid }">${m.modlename }</option>
										</c:forEach>
				                	</select>
								</td>
								<td>设备类型：</td>
	...
	function initEditUser(getSelection){
		...
		$('#username').val(getSelection.ename);
		$('#memory').val(getSelection.memory);
